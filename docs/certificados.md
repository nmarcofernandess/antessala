...
> Guia canônico para distribuir builds assinados do FlowKit (e forks como EscalaFlow). Foco em **produção** (GitHub Releases + auto-update) e **testes de builds ad-hoc**.

Status atual (2026-06): o FlowKit publica GitHub Releases por tag `v*` via GitHub Actions (mac + win), mas os builds ainda são **ad-hoc** (sem certificado de produção) — exigem bypass manual do Gatekeeper/SmartScreen.

Conteúdo mantido em sync com EscalaFlow (FlowKit é o template de referência canônica; nomes de app e paths são ajustados por fork).
...
- Estado ad-hoc atual em `electron-builder.yml`:
  ```yaml
  mac:
    identity: "-"
    ...
    notarize: false
  ```
- Config alvo quando entrar assinatura/notarização:
  ```yaml
  mac:
...
xattr -cr "/Applications/FlowKit.app" && open "/Applications/FlowKit.app"
```
...
## Estado atual dos repositórios (FlowKit + forks)

- `build/entitlements.mac.plist`: presente e correto para Electron (JIT + memória).
- `electron-builder.yml`: aponta `publish` para GitHub Releases em `nmarcofernandess/flowkit`.
- `.github/workflows/dist.yml`: empacotamento manual ad-hoc para artifacts de CI.
- `.github/workflows/release.yml`: release por tag `v*`, com macOS + Windows, `latest-mac.yml`, `latest.yml`, DMG/ZIP/EXE e blockmaps publicados direto no GitHub Release.
- **Falta em produção hoje**: secrets de Apple (notarize) e certificado Windows (OV/EV pfx ou Azure Artifact Signing) nos workflows de release público. Builds oficiais ainda são efetivamente ad-hoc e não notarizados. (Veja `electron-builder.yml`: `notarize: false`, `identity: "-"` para ad-hoc.)
...
Mantido em sync com `docs/release.md`, `.github/workflows/release.yml` e `electron-builder.yml`. (Entitlements atuais em `build/entitlements.mac.plist` + `identity: "-" + notarize: false` no yml para builds ad-hoc.)
