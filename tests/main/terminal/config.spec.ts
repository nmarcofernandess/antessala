...
describe('terminal harness config', () => {
  ...
  it('does not silently fall back when config storage fails', async () => {
    queryOne.mockRejectedValueOnce(new Error('db offline'))
    const { getTerminalHarnessConfig } = await import('../../../src/main/terminal/config')

    await expect(getTerminalHarnessConfig()).rejects.toThrow('Nao foi possivel carregar a configuracao do terminal: db offline')
  })
