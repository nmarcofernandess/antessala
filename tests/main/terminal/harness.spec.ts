...
describe('terminal harness', () => {
  ...
  it('returns audit failure metadata without hiding the command result', async () => {
    vi.mocked(execute).mockRejectedValueOnce(new Error('audit table locked'))
    ...
    expect(result.result.stdout).toBe('audited')
