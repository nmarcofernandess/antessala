...
describe('headless Electron mode', () => {
  it('keeps the app running without showing the main window only when explicitly enabled', () => {
    expect(isHeadlessRun({ FLOWKIT_HEADLESS: '1' })).toBe(true)
    expect(shouldShowMainWindow({ FLOWKIT_HEADLESS: '1' })).toBe(false)
    expect(isHeadlessRun({ FLOWKIT_HEADLESS: 'true' })).toBe(false)
