...
import { isSttSidecarExecutable, transcribeWithSidecar } from '../../src/main/stt/stt-bridge'
...
describe('stt bridge', () => {
  it('does not report a non-executable sidecar as available on Unix', () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), 'stt-sidecar-perm-'))
    const bin = path.join(dir, 'fake-stt')
    writeFileSync(bin, '#!/bin/sh\n')
    if (process.platform !== 'win32') chmodSync(bin, 0o644)

    expect(isSttSidecarExecutable(bin)).toBe(process.platform === 'win32')
  })
