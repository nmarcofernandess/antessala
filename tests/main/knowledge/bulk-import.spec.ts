...
vi.mock('../../../src/main/ia/metadata-generator', () => ({
  generateRagMetadata: vi.fn(),
}))
...
describe('bulk RAG import', () => {
  ...
  it('generates AI metadata per file when auto_metadata is enabled', async () => {
    const { runBulkRagImport } = await import('../../../src/main/knowledge/bulk-import')
    ...
    const job = createJob({ type: 'bulk_rag_import', label: 'Bulk AI metadata' })
    const summary = await runBulkRagImport({
      path: tmpDir,
      group_name: 'Grupo IA',
      auto_enrich: false,
    ...
    expect(vi.mocked(generateRagMetadata)).toHaveBeenCalledTimes(1)
  ...
  it('still imports and records a warning when AI metadata fails', async () => {
    ...
    const job = createJob({ type: 'bulk_rag_import', label: 'Bulk AI metadata fail' })
    const summary = await runBulkRagImport({
      path: tmpDir,
      group_name: 'Grupo IA Falha',
      auto_enrich: false,
      auto_metadata: true,
    }, job.id)

    // File still imports, but the AI-metadata miss is recorded (not a silent clean "done").
    expect(summary.imported_files).toBe(1)
    expect(summary.errors.some((entry) => entry.message.startsWith('AI metadata'))).toBe(true)
    expect(getJob(job.id)?.status).toBe('done')
  })
