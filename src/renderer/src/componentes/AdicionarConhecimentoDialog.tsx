...
import { AlertCircle, ArrowLeft, ChevronDown, FolderOpen, Loader2, Sparkles, Upload, FileText, Pause, Play, X } from 'lucide-react'
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
interface Props {
  ...
  iaRouteMessage?: string
  iaRouteAction?: string
}

export function AdicionarConhecimentoDialog({
  open,
  onOpenChange,
  onSaved,
  iaDisponivel,
  ...
  useEffect(() => {
    if (!bulkJob || ['done', 'partial', 'failed', 'cancelled'].includes(bulkJob.status)) return
    ...
    const timer = window.setInterval(async () => {
      try {
        ...
        if (result.job.status === 'done') {
          ...
          toast.warning('Importação em massa concluída com avisos.', {
            description: String(
              result.job.metadata.warning_message
                ?? result.job.metadata.enrichment_error
                ?? result.job.error_message
                ?? 'Verifique os arquivos do job.',
            ),
          })
          onSaved()
  ...
  const iaMetadataMessage = iaDisponivel
    ? null
    : iaRouteMessage || 'Metadados por IA indisponíveis para esta rota.'
  const iaMetadataAction = iaDisponivel
    ? null
    : iaRouteAction || 'Configure uma rota pronta em Configurações > IA e Modelos.'
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-x-hidden overflow-y-auto">
        <DialogHeader className="pr-8">
          ...
          <DialogDescription className="sr-only">
            Selecione um arquivo, revise o texto extraído e salve o conhecimento no RAG.
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="flex min-w-0 max-w-full flex-col gap-3 overflow-hidden rounded-md border p-3">
                ...
                {bulkJob && (
                  <div className="flex flex-col gap-2">
                    ...
                    {!['done', 'partial', 'failed', 'cancelled'].includes(bulkJob.status) && (
                ...
                <Button
                  ...
                  disabled={salvando || Boolean(bulkJob && !['done', 'partial', 'failed', 'cancelled'].includes(bulkJob.status))}
                >
        ...
        {step === 2 && (
          <>
            ...
            {iaMetadataMessage && (
              <div className="flex gap-2 rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-sm text-amber-900 dark:text-amber-200">
                <AlertCircle className="mt-0.5 size-4 shrink-0" />
                <div className="min-w-0">
                  <p className="font-medium">Metadados automáticos indisponíveis</p>
                  <p className="text-xs">{iaMetadataMessage}</p>
                  {iaMetadataAction && (
                    <p className="mt-1 text-xs">{iaMetadataAction}</p>
                  )}
                </div>
              </div>
            )}
            ...
            <div className="flex flex-col gap-1.5">
              <div className="flex items-center justify-between">
                ...
                {!gerando.titulo && conteudo && (
                  <Button
                    variant="ghost"
                    size="icon"
                    className="size-6"
                    disabled={!iaDisponivel || alguemGerando}
                    onClick={() => handleGerarCampo('titulo')}
                    title={iaDisponivel ? 'Regenerar título com IA' : 'IA indisponível para metadados'}
              ...
              <div className="flex items-center justify-between">
                ...
                {!gerando.quando && conteudo && (
                  <Button
                    ...
                    title={iaDisponivel ? 'Regenerar sugestão com IA' : 'IA indisponível para metadados'}
