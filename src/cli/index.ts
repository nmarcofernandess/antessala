...
const NOT_RUNNING_MSG =
  'FlowKit nao esta rodando. Abra o app primeiro.'
...
function formatServerError(status: number, text: string): string {
  try {
    ...
    return `Erro do FlowKit (${status}): ${message}${provider}${model}${actionLine}`
  } catch {
    return `Erro do FlowKit (${status}): ${text}`
  }
}
...
program
  .name('flowkit')
  .description('FlowKit CLI — chat, search and manage your knowledge base')
  .version('1.0.0')
...
program
  ...
  .action(async (message?: string) => {
    ...
    console.log('FlowKit chat conectado e IA validada. Digite "sair" para encerrar.')
...
program
  .command('status')
  .description('Status do FlowKit e estatisticas do knowledge base')
  .action(async () => {
    ...
    console.log(`\n  FlowKit v${health.version ?? '?'}  (${health.tools ?? '?'} tools)`)

    const stats = (await callTool('status_sistema', {})) as ToolResponse
    if (stats?.payload && typeof stats.payload === 'object') {
      const s = stats.payload as unknown as Record<string, unknown>
      console.log(`  Fontes:     ${s.fontes ?? '?'}`)
      console.log(`  Chunks:     ${s.chunks ?? '?'}`)
      console.log(`  Entidades:  ${s.entidades ?? '?'}`)
      console.log(`  Memorias:   ${s.memorias ?? '?'}`)
...
rag
  ...
  .option('--ai-metadata', 'Gera titulo/contexto de consulta com IA durante a importacao')
  .action(async (targetPath: string, options: { group?: string; recursive?: boolean; wait?: boolean; enrich?: boolean; aiMetadata?: boolean }) => {
    ...
    const data = await postJson('/rag/import', {
      ...
      ...(typeof options.aiMetadata === 'boolean' ? { auto_metadata: options.aiMetadata } : {}),
    ...
    if (!options.wait) {
      console.log(`Use: flowkit jobs list  ou  flowkit jobs cancel ${data.job.id}`)
      return
    }
    ...
    while (true) {
      ...
      if (['done', 'partial', 'failed', 'cancelled'].includes(job.status)) {
        ...
        if (job.status === 'partial') process.exit(2)
        return
      }
...
terminal
  .command('open-cli')
  .description('Abre o FlowKit CLI no Terminal do sistema')
    ...
    printJson({
      mcpServers: {
        flowkit: {
          ...
          cwd: process.cwd(),
          env: {
            FLOWKIT_TOOL_SERVER: TOOL_SERVER,
          },
        },
      },
    })
