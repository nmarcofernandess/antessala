...
import { resolveAiTerminalCliCommand } from './cli-command'
import { launchSystemTerminalCommand, openSystemTerminalWithScript } from './open-system-terminal'
...
export async function runTerminalCommandWithConfig(
  ...
  const audit = await recordTerminalCommand({
    source,
    command: normalized.command,
    cwd: normalized.cwd,
  ...
  return { status: isTerminalExecSuccess(result) ? 'ok' : 'error', result, config, audit }
}
  ...
  void runTerminalCommandWithConfig(normalized, source, { signal: controller.signal })
    .then(({ result, audit }) => {
      ...
      if (!isTerminalExecSuccess(result)) {
        updateJob(job.id, { done: 1, metadata: { result, audit } })
        failJob(
          job.id,
...
export function buildOpenCliCommand(input?: { command?: string; cwd?: string }): TerminalOpenCliResult {
  ...
  const command = input?.command?.trim() || resolveAiTerminalCliCommand(process.cwd()).command
...
export async function openCliInSystemTerminal(input?: { command?: string; cwd?: string }): Promise<TerminalOpenCliResult> {
  ...
  if (process.platform === 'darwin') {
    ...
    await launchSystemTerminalCommand('osascript', ['-e', script])
  ...
  const args = process.platform === 'win32'
    ...
    : ['-e', shellCommand]
  await launchSystemTerminalCommand(terminal, args)
  return { ...base, opened: true }
}
