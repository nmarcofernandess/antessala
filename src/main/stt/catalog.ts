...
export function getSttModelPath(modelId: SttModelId): string {
  const bundledPath = getBundledSttModelPath(modelId)
  if (bundledPath && isSttModelCompleteAtPath(modelId, bundledPath)) return bundledPath
  return getSttModelInstallPath(modelId)
}

function isSttModelCompleteAtPath(modelId: SttModelId, modelPath: string): boolean {
...
export function getBundledSttModelPath(modelId: SttModelId): string | null {
  const bundledBase = getBundledSttModelBaseDir()
  return bundledBase ? path.join(bundledBase, STT_MODELS[modelId].filename) : null
}
...
export function getSttModelSource(modelId: SttModelId): 'bundled' | 'user' | 'missing' {
  const bundledPath = getBundledSttModelPath(modelId)
  if (bundledPath && isSttModelCompleteAtPath(modelId, bundledPath)) return 'bundled'
  if (isSttModelCompleteAtPath(modelId, getSttModelInstallPath(modelId))) return 'user'
  return 'missing'
}

export function isSttModelDownloaded(modelId: SttModelId): boolean {
  return getSttModelSource(modelId) !== 'missing'
}
