...
export function isSttSidecarExecutable(sidecarPath: string): boolean {
  try {
    const stat = fs.statSync(sidecarPath)
    if (!stat.isFile()) return false
    fs.accessSync(sidecarPath, process.platform === 'win32' ? fs.constants.F_OK : fs.constants.X_OK)
    return true
  } catch {
    return false
  }
...
export function isSttSidecarAvailable(): boolean {
  return isSttSidecarExecutable(getSttSidecarPath())
}
...
export async function transcribeWithSidecar(input: {
  ...
  if (!isSttSidecarExecutable(sidecarPath)) {
    throw new Error(`Binario de ditado local nao encontrado ou sem permissao de execucao: ${sidecarPath}`)
  }
