...
export function updateJob(id: string, patch: {
  ...
  const currentIsFinal = isFinalJobStatus(job.status)
  const nextStatus = currentIsFinal && patch.status && patch.status !== job.status
    ? job.status
...
export function finishJob(id: string, metadata?: Record<string, unknown>, status: 'done' | 'partial' = 'done'): AppJob {
  const current = getJob(id)
  if (current && isFinalJobStatus(current.status)) return current

  const job = updateJob(id, {
    status,
    done: current?.progress.total,
    metadata,
    error_message: status === 'partial' ? (current?.error_message ?? null) : null,
  })
  const next: AppJob = {
    ...job,
    finished_at: nowIso(),
