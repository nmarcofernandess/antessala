...
import { resolveProviderApiKey, resolveModel } from './config'
...
const _electronModule = _require('electron') as Partial<typeof import('electron')> | string
const _BW = typeof _electronModule === 'object' && _electronModule !== null
  ? _electronModule.BrowserWindow
  : undefined
...
let _streamEventSink: ((event: IaStreamEvent) => void) | null = null

function emitStream(event: IaStreamEvent): void {
  _streamEventSink?.(event)
...
function shouldEnableAiDevTools() {
    const explicit = process.env.FLOWKIT_AI_DEVTOOLS?.trim()
...
function estimateToolSeconds(_toolName: string, _input: unknown): number | undefined {
    // No long-running tools in FlowKit generic mode
    return undefined
}
...
export const buildUserContentForTest = buildUserContent
...
export const __iaClienteTestables = {
    ...
    setStreamEventSink(sink: ((event: IaStreamEvent) => void) | null) {
        _streamEventSink = sink
    },
...
async function _callWithVercelAiSdkTools(
  ...
  try {
    ...
    const fullSystemPrompt = options?.systemPromptOverride ?? await buildFullSystemPrompt(contexto, currentMsg)
    ...
    if (err.statusCode === 429 || err.status === 429 ||
      ...
      throw new Error('Limite de requisicoes excedido. Aguarde alguns segundos e tente novamente.')
    }
    ...
    if (err.statusCode === 401 || err.statusCode === 403 ||
      ...
      throw new Error('API key invalida ou sem permissao. Verifique sua chave em Configuracoes > IA.')
    }
    throw err
  }
}

async function _callWithVercelAiSdkToolsStreamBridge(
    providerLabel: 'gemini' | 'openrouter',
    config: IaConfiguracao,
    currentMsg: string,
    historico: IaMensagem[],
    ...
    const { systemPrompt, meta: contextMeta } = await buildSystemPromptWithMeta(contexto, currentMsg)
    if (contextMeta) {
        emitStream({ type: 'context-meta', stream_id: streamId, meta: contextMeta })
    }

    emitStream({ type: 'start-step', stream_id: streamId, step_index: 0 })

    try {
        const result = await _callWithVercelAiSdkTools(
            providerLabel,
            config,
            currentMsg,
            historico,
        ...
        if (result.resposta) {
            emitStream({ type: 'text-delta', stream_id: streamId, delta: result.resposta })
        }
        emitStream({ type: 'step-finish', stream_id: streamId, step_index: 0 })
        emitStream({ type: 'finish', stream_id: streamId, resposta: result.resposta, acoes: result.acoes })

        return result
    } catch (err: any) {
        const message = err instanceof Error ? err.message : String(err)
        emitStream({ type: 'error', stream_id: streamId, message })
        throw err
    }
}
...
async function _callWithVercelAiSdkToolsStreaming(
    ...
    try {
        ...
        if (err.statusCode === 429 || err.status === 429 ||
            ...
            err.message?.toLowerCase().includes('quota') ||
            err.message?.toLowerCase().includes('too many requests')) {
          const friendlyMsg = 'Limite de requisicoes excedido. Aguarde alguns segundos e tente novamente.'
          emitStream({ type: 'error', stream_id: streamId, message: friendlyMsg })
        ...
        if (err.statusCode === 401 || err.statusCode === 403 ||
            ...
            err.message?.toLowerCase().includes('invalid api key') ||
            err.message?.toLowerCase().includes('api key not valid')) {
          const friendlyMsg = 'API key invalida ou sem permissao. Verifique sua chave em Configuracoes > IA.'
          emitStream({ type: 'error', stream_id: streamId, message: friendlyMsg })
          throw new Error(friendlyMsg)
        }
...
export async function iaEnviarMensagemStream(
    ...
    if (route.provider === 'local') {
        ...
        return localLlmChat(mensagem, historico, streamId, contexto, conversa_id, anexos, { modelId: asLocalModelId(route.model) })
    }
    ...
    if (config.provider === 'gemini' && !isGeminiCloudApiEnabled()) {
        throw new Error('Gemini API direta desativada por FLOWKIT_GEMINI_ENABLE=0. Troque para OpenRouter ou modelo local nas Configurações.')
    }
    ...
    if (config.provider === 'openrouter') {
        ...
        return _callWithVercelAiSdkToolsStreamBridge('openrouter', { ...config, api_key: apiKey }, mensagem, historico, streamId, contexto, (modelo) => openrouter(modelo), conversa_id, anexos)
    }
...
export async function iaEnviarMensagem(
    ...
    if (route.provider === 'local') {
        const { localLlmChat, asLocalModelId } = await import('./local-llm')
        return localLlmChat(mensagem, historico, 'non-stream', contexto, conversa_id, anexos, { modelId: asLocalModelId(route.model) })
    }
...
export async function iaTestarConexao(
    ...
    if (provider === 'gemini' && !isGeminiCloudApiEnabled()) {
        throw new Error('Gemini API direta desativada por FLOWKIT_GEMINI_ENABLE=0.')
    }
    try {
        if (provider === 'gemini') {
            const google = createGoogleGenerativeAI({ apiKey })
