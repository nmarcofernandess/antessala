...
import {
  ...
  validateLocalLlamaServerVision,
...
export const LOCAL_MODELS = {
  'gemma-4-e2b-it-q4': {
    ...
    // Projetor multimodal (visão). É um GGUF separado no mesmo repo unsloth.
    // Baixado sob demanda; a visão só liga se detectVisionSupport() provar o runtime.
    mmproj: {
...
/** Narrows a route-resolved model string to a LocalModelId, throwing if it isn't a known local model. */
export function asLocalModelId(value: string): LocalModelId {
  if (Object.prototype.hasOwnProperty.call(LOCAL_MODELS, value)) return value as LocalModelId
 ...
 * baixado em ".../FlowKit/models" não é encontrado. Usar o nome do produto
 * mantém o mesmo dir em dev e empacotado.
 *
 * Invariante: APP_CONFIG.name precisa bater com o `productName` do
 * electron-builder.yml (que define o userData REAL do empacotado), senão o dir
 * aponta pra fora do userData. Travado por teste em model-dir.spec.ts.
 *
 * Exportada para teste (getModelDir/getModelPath são internos ao módulo).
 */
export function getModelDir(): string {
  if (process.env.FLOWKIT_LOCAL_MODELS_DIR) {
    return process.env.FLOWKIT_LOCAL_MODELS_DIR
  }
...
function getMmprojConfig(modelId: LocalModelId): { filename: string; url: string; size_bytes: number } | null {
  const entry = LOCAL_MODELS[modelId] as { mmproj?: { filename: string; url: string; size_bytes: number } }
  return entry.mmproj ?? null
}

export function getMmprojPath(modelId: LocalModelId): string | null {
  const mmproj = getMmprojConfig(modelId)
  return mmproj ? path.join(getModelDir(), mmproj.filename) : null
}

function isMmprojDownloaded(modelId: LocalModelId): boolean {
  const mmproj = getMmprojConfig(modelId)
  const mmprojPath = getMmprojPath(modelId)
  if (!mmproj || !mmprojPath || !fs.existsSync(mmprojPath)) return false
  return fs.statSync(mmprojPath).size > mmproj.size_bytes * 0.95
}
...
export type VisionSupport = { ok: true } | { ok: false; reason: string }

// Detecção honesta de visão local: precisa do mmproj baixado E do llama-server
// carregando-o sem erro de arquitetura (probe real com imagem minúscula).
// Sem isso, retorna ok:false com motivo — a Maiá nunca finge enxergar.
export async function detectVisionSupport(preferredModelId?: LocalModelId): Promise<VisionSupport> {
  let modelId: LocalModelId
  try {
    modelId = getActiveLocalModelId(preferredModelId)
  } catch {
    return { ok: false, reason: 'Modelo local não baixado para visão.' }
  }

  // getMmprojPath é null exatamente quando o modelo não tem projetor no catálogo.
  const mmprojPath = getMmprojPath(modelId)
  if (!mmprojPath) {
    return { ok: false, reason: 'Modelo local não tem projetor de visão (mmproj).' }
  }
  if (!isMmprojDownloaded(modelId)) {
    return { ok: false, reason: 'Projetor de visão (mmproj) não baixado.' }
  }

  try {
    resetIdleTimer()
    await validateLocalLlamaServerVision({ modelId, modelPath: getModelPath(modelId), mmprojPath })
    return { ok: true }
  } catch (err: any) {
...
export async function localLlmChat(
  ...
  conversa_id?: string,
  anexos?: IaAnexo[],
  options?: { modelId?: LocalModelId },
): Promise<{ resposta: string; acoes: ToolCall[] }> {
  let targetModelId: LocalModelId
  try {
    targetModelId = getActiveLocalModelId(options?.modelId)
  } catch (err: any) {
  ...
  if (shouldUseLlamaServerRuntime(targetModelId)) {
    ...
    try {
      const result = await localLlamaServerChat({
        ...
        anexos,
        // Anexa --mmproj só com o projetor baixado (existência+tamanho). A prova
        // de que o runtime REALMENTE enxerga vem do detectVisionSupport/probe e do
        // gate visionEnabled em buildMessages — não desta checagem de arquivo.
        mmprojPath: isMmprojDownloaded(targetModelId) ? getMmprojPath(targetModelId) : null,
      })
  ...
  try {
    loaded = await ensureModelLoaded(targetModelId)
  } catch (err: any) {
