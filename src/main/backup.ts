...
export function parseBackupFile(filepath: string): { meta: SnapshotMeta; dados: Record<string, unknown[]> } {
  ...
  if (isZip) {
    ...
    if (meta?.app !== 'flowkit') throw new Error('Arquivo de backup invalido. Selecione um backup do FlowKit.')
  ...
  if (parsed?._meta?.app === 'flowkit') {
    // Flat snapshot format: { _meta, knowledge_sources: [...], ia_conversas: [...], ... }
  ...
  if (parsed?.dados && typeof parsed.dados === 'object') {
    ...
    if (parsed._meta?.app !== 'flowkit') {
      throw new Error('Arquivo de backup invalido. Selecione um arquivo exportado pelo FlowKit.')
    }
...
async function repairRestoredOperationalState(): Promise<number> {
  // No FlowKit-specific repairs needed (yet)
  log('Repair: no-op (FlowKit generic)')
  return 0
}
...
export async function createSnapshot(
  ...
  try {
    ...
    const filename = `flowkit-backup-${ts}.zip`
...
export async function listSnapshots(userData: string): Promise<SnapshotInfo[]> {
  ...
  const files = fs.readdirSync(dir).filter((f) =>
    (f.startsWith('flowkit-backup-') && f.endsWith('.zip')) ||
...
export async function restoreSnapshot(
  ...
  if (meta?.app !== 'flowkit') {
    throw new Error('Arquivo invalido: nao e um backup FlowKit')
  }
