...
import type { ImportResult } from '../../shared/importer-types'
...
async function metadataForImportedFile(
  rootPath: string,
  filePath: string,
  imported: ImportResult,
  autoMetadata: boolean,
  summary: BulkRagImportSummary,
): Promise<{ title: string; metadata: Record<string, unknown> }> {
  const title = titleForFile(rootPath, filePath)
  if (!autoMetadata) return { title, metadata: {} }

  try {
    if (imported.type !== 'text') return { title, metadata: {} }

    const { generateRagMetadata } = await import('../ia/metadata-generator')
    const generated = await generateRagMetadata({
      texto: imported.data.text,
      fileNameFallback: title,
    })

    return {
      title: generated.titulo || title,
      metadata: {
        context_hint: generated.quando_consultar,
        quando_consultar: generated.quando_consultar,
        ai_metadata: true,
        ai_metadata_route: {
    ...
    summary.errors.push({
      path: filePath,
      message: `AI metadata ignorada: ${errorMessage(error)}`,
    })
...
export function startBulkRagImport(input: BulkRagImportInput): AppJob {
  ...
  const job = createJob({
    ...
    metadata: {
      ...
      auto_metadata: input.auto_metadata,
    },
  })
...
export async function runBulkRagImport(
  ...
  try {
    ...
    // AI-metadata failures don't increment failed_files (the file still imports), so surface them
    // explicitly instead of letting the persistent record report a silent clean "done".
    const metadataWarnings = summary.errors.filter((entry) => entry.message.startsWith('AI metadata')).length
    const warnings: string[] = []
    if (summary.failed_files > 0) warnings.push(`${summary.failed_files} arquivo(s) com erro`)
    if (metadataWarnings > 0) warnings.push(`${metadataWarnings} arquivo(s) sem metadados de IA`)
    if (enrichmentError) warnings.push(`enrichment nao executado: ${enrichmentError}`)
    await updateKnowledgeImportJob(importJob.id, {
      status: cancelled ? 'cancelled' : 'done',
      failed_files: summary.failed_files,
      chunks_created: summary.chunks_count,
      error_message: warnings.length > 0 ? `Importacao concluida; ${warnings.join('; ')}.` : null,
      finished: true,
    })
